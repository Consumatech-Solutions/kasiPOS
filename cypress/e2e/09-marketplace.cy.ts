...
describe("Marketplace", () => {
  ...
  it("searches order by code", () => {
    cy.findByPlaceholderText(/enter order code/i).type("MK-10001", {
      force: true,
    });
  ...
  it("places marketplace order on behalf of customer", () => {
    ...
    cy.findByRole("button", { name: /add customer/i }, { timeout: 30_000 })
      .should("be.visible")
      .and("not.be.disabled");
    cy.contains(/fresh mart/i).should("be.visible");
    ...
    cy.contains("tr", /alice mokoena/i, { timeout: 30_000 }).click({
      force: true,
    });
