...
describe('Customers', () => {
  ...
  it('creates customer with zero loyalty points', () => {
    ...
    cy.findByLabelText(/loyalty points/i).clear().type('0');
  ...
  it('edits customer details', () => {
    ...
    cy.findByLabelText(/customer name/i).clear().type('Brian Updated');
    cy.findByLabelText(/contact/i).clear().type('0821999000');
  ...
  it('updates loyalty points after completed purchase', () => {
    ...
    cy.get('@cashDialog').within(() => {
      cy.contains('button', /^exact$/i, { timeout: 15_000 }).click({ force: true });
    ...
    cy.wait('@createTransaction').then((interception) => {
      const total = Number((interception.request.body as { total?: number }).total ?? 0);
      const earnedPoints = Math.max(1, Math.floor(total / 10));
      cy.wrap(startingPoints + earnedPoints, { log: false }).as('expectedPoints');
    });

    CustomersPage.visit();
    CustomersPage.waitUntilLoaded();
    cy.get<number>('@expectedPoints').then((expectedPoints) => {
      cy.findByPlaceholderText(/search by name or contact/i, { timeout: 20_000 }).clear().type('Alice');
  ...
  it('handles customer API failure', () => {
    ...
    cy.findByRole('dialog', { name: /add customer/i, timeout: 15_000 }).should('be.visible');
