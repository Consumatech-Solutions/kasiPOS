...
/** Re-query the cart row each time to avoid detached DOM when React re-renders after quantity changes. */
function cartRow(productName: string) {
  return cy.get(`[data-testid="pos-cart-line"][data-product-name="${productName}"]`);
}

/** Qty > 1 via cart row plus buttons (catalog `addProduct` loop is flaky under React Strict Mode in dev). */
...
describe('POS sale flow', () => {
  ...
  it('completes sale, shows receipt, and clears cart', () => {
    ...
    cy.contains(/sale complete!/i).should('be.visible');
    cy.findByRole('dialog', { name: /receipt/i }, { timeout: 25_000 }).should('be.visible');
    cy.findByRole('dialog', { name: /receipt/i }).within(() => {
      cy.findByText(/^receipt$/i).should('be.visible');
    });
    cy.findByRole('dialog', { name: /receipt/i })
