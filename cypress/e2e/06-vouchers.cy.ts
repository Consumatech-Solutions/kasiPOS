...
describe('Vouchers', () => {
  ...
  it('creates percentage voucher', () => {
    ...
    cy.findByLabelText(/percentage/i).clear().type('20');
    cy.findByLabelText(/minimum purchase/i).clear().type('10');
  ...
  it('creates fixed amount voucher', () => {
    ...
    cy.findByLabelText(/amount/i).clear().type('25');
    cy.findByLabelText(/minimum purchase/i).clear().type('50');
    cy.findByRole('button', { name: /^create$/i }).click({ force: true });
    cy.wait('@createVoucher');
  ...
  it('applies voucher code from POS and reflects discount', () => {
    ...
    cy.get('#voucherCode', { timeout: 15_000 }).clear().type('SAVE10');
    cy.findByRole('button', { name: /find voucher/i }).should('be.enabled').click();
    cy.wait('@validateVoucher').its('response.body').should('have.property', 'valid', true);
    cy.findByRole('alert', { timeout: 15_000 }).should('contain.text', 'Voucher Found!');
    cy.findByRole('button', { name: /apply discount/i }).should('be.enabled').click();
    cy.get('#voucherCode').should('not.exist');
    cy.contains('Voucher applied', { matchCase: false, timeout: 20_000 }).should('be.visible');
  });
  ...
  it('rejects inactive voucher in POS validation', () => {
    ...
    PosPage.selectCustomerFromDialog('Alice Mokoena');
    PosPage.openVoucherDialog();
    cy.get('#voucherCode', { timeout: 15_000 }).clear().type('INACTIVE5');
    cy.findByRole('button', { name: /find voucher/i }).should('be.enabled').click();
    cy.wait('@validateVoucher').its('response.body').should('have.property', 'valid', false);
