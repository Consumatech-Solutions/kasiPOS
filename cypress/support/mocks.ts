...
function isVoucherValid(
  ...
  if (cartTotal < voucher.minPurchase) {
    return {
      valid: false,
      message: `Minimum purchase is R${voucher.minPurchase.toFixed(2)}.`,
    };
  }
  if (
...
/**
 * Cross-origin fetch(HEAD) from the app origin to API_BASE_URL must include ACAO
 * or the browser rejects the response and offline-detector treats the app as offline.
 */
...
export function registerApiMocks(options: MockApiOptions): MockApiControls {
  ...
  const apiOrigin = stripTrailingSlashes(apiBaseUrl);
  cy.intercept("OPTIONS", `${apiOrigin}/**`, (req) => {
    if (replyOfflineIfNeeded(req)) return;
    corsReply(req, { statusCode: 204, body: "" });
  });
  cy.intercept("OPTIONS", apiOrigin, (req) => {
    if (replyOfflineIfNeeded(req)) return;
    corsReply(req, { statusCode: 204, body: "" });
  ...
  cy.intercept("GET", `${apiBaseUrl}/auth/profile`, (req) => {
    if (replyOfflineIfNeeded(req)) return;
    corsReply(req, { statusCode: 200, body: clone(options.seedAuth.user) });
  }).as("getAuthProfile");
  ...
  cy.intercept("DELETE", `${apiBaseUrl}/categories/*`, (req) => {
    ...
    if (hasProducts) {
      corsReply(req, {
        statusCode: 400,
        body: { message: "Cannot delete category with products." },
      });
      return;
    }
  ...
  cy.intercept("GET", `${apiBaseUrl}/transactions*`, (req) => {
    ...
    if (search)
      rows = rows.filter((row) => row.id.toLowerCase().includes(search));
    if (date)
  ...
  cy.intercept("POST", `${apiBaseUrl}/transactions`, (req) => {
    ...
    for (const item of payload.items) {
      state.products = state.products.map((row) => {
        ...
        return {
          ...row,
          stock: Math.max(0, Number(row.stock) - Number(item.quantity)),
          updatedAt: isoNow(),
        };
      });
    }
  ...
  cy.intercept("POST", `${apiBaseUrl}/vouchers`, (req) => {
    ...
    const created = {
      ...
      maxUsesPerCustomer:
        (payload.maxUsesPerCustomer as number | null | undefined) ?? null,
      currentUses: 0,
      storeId: options.seedAuth.store.id,
      createdAt: isoNow(),
      updatedAt: isoNow(),
  ...
  cy.intercept("PATCH", `${apiBaseUrl}/vouchers/*`, (req) => {
    ...
    if (!existing) {
      corsReply(req, {
        statusCode: 404,
        body: { message: "Voucher not found" },
      });
      return;
    }
    const updated = {
  ...
  cy.intercept("POST", `${apiBaseUrl}/vouchers/validate`, (req) => {
    ...
    if (!voucher) {
      corsReply(req, {
        statusCode: 200,
        body: { valid: false, message: "Voucher code is invalid." },
      });
      return;
    }
    ...
    if (!validity.valid) {
      corsReply(req, {
        statusCode: 200,
        body: { valid: false, message: validity.message },
      });
      return;
    }
  ...
  cy.intercept("POST", `${apiBaseUrl}/parcels/*/receive`, (req) => {
    ...
    const updated = {
      ...
      collectionCode:
        ...
        `COLL-${Math.floor(Math.random() * 9000 + 1000)}`,
      dateReceived: now,
      updatedAt: now,
    };
  ...
  cy.intercept("POST", `${apiBaseUrl}/parcels/*/collect`, (req) => {
    ...
    if (
      String(existing.collectionCode ?? "") !== String(payload.collectionCode)
    ) {
      corsReply(req, {
        statusCode: 400,
        body: { message: "Collection code does not match." },
      });
      return;
    }
    const updated = {
  ...
  cy.intercept("GET", `${apiBaseUrl}/marketplace-stores*`, (req) => {
    ...
    const rows = activeOnly
      ? state.marketplaceStores.filter((row) => row.isActive)
      : state.marketplaceStores;
    corsReply(req, { statusCode: 200, body: rows });
  ...
  cy.intercept("GET", `${apiBaseUrl}/marketplace-orders/search*`, (req) => {
    ...
    corsReply(req, { statusCode: 200, body: found });
