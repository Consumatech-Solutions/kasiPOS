...
const API_BASE_URL = (Cypress.env('API_BASE_URL') as string) ?? 'http://localhost:3000';
...
function applySessionToStorage(win: Window, session: SeedAuthSession) {
  ...
  win.localStorage.setItem('kasi-pos-settings', JSON.stringify(session.settings));
  win.localStorage.setItem('kasiPOS_hardwareSetupCompleted', 'true');
  // Reliable E2E markers (read in SettingsProvider). Session survives `cy.clearLocalStorage()` between tests.
...
function seedObjectStore(tx: IDBTransaction, storeName: string, rows: unknown[]) {
...
function writeSeedToIndexedDb(win: Window, seed: SeedData, session: SeedAuthSession): Promise<void> {
  return ensureReadyDatabase(win).then(
    (db: IDBDatabase) =>
      new Cypress.Promise<void>((resolve, reject) => {
        const tx = db.transaction(
          ['stores', 'products', 'customers', 'transactions', 'vouchers', 'categories', 'stockAdjustments', 'parcels'],
          'readwrite',
        );

        tx.onerror = () => reject(tx.error ?? new Error('Failed to write IndexedDB seed'));
        tx.oncomplete = () => {
          db.close();
          resolve();
        };
...
Cypress.Commands.add('setOffline', () => {
  ...
  cy.window().then((win) => {
    const bridge = (win as Window & { __KASI_POS_E2E_OFFLINE?: (forcedOffline: boolean) => void }).__KASI_POS_E2E_OFFLINE;
    bridge?.(true);
...
Cypress.Commands.add('setOnline', () => {
  if (!mockControls) {
    throw new Error('setOnline() called before cy.setupScenario() — API mocks are not registered.');
  }
  mockControls.setOffline(false);
  cy.window().then((win) => {
    const bridge = (win as Window & { __KASI_POS_E2E_OFFLINE?: (forcedOffline: boolean) => void }).__KASI_POS_E2E_OFFLINE;
  ...
  cy.waitUntil(
    () =>
      cy.document({ log: false }).then((doc) => {
        ...
        if (!pathname.startsWith('/marketplace') && !pathname.startsWith('/boph')) return true;
        return !doc.querySelector('.opacity-60.pointer-events-none.select-none');
      }),
    {
      timeout: 20_000,
      interval: 250,
...
Cypress.Commands.add('waitForAppReady', (expectedPath: string | RegExp = '/') => {
  cy.waitUntil(
    () =>
...
Cypress.Commands.add('visitApp', (path = '/') => {
  ...
  return cy.get<SeedAuthSession>('@seedAuthSession').then((session) => {
    ...
    if (normalized === '/' || normalized === '/catalogue' || normalized === '/inventory') {
      cy.wait(200);
      load();
    }
...
Cypress.Commands.add('setupScenario', (input = {}) => {
  ...
  const chain = cy
    ...
    .then((sessionObj) => {
      const session = sessionObj as SeedAuthSession;
      cy.wrap(session, { log: false }).as('seedAuthSession');
      return cy.task('loadSeedData', input).then((seedObj) => ({ session, seed: seedObj as SeedData }));
    })
    .then(({ session, seed }) => {
      cy.wrap(seed, { log: false }).as('seedData');
