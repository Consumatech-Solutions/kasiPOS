...
describe("cart-storage", () => {
  ...
  describe("cartToStorage", () => {
    ...
    it("keeps non-blob imageUrl", () => {
      const cart = new Map([
        ["p1", sampleItem({ imageUrl: "https://cdn.example/x.png" })],
      ]);
      const parsed = JSON.parse(cartToStorage(cart)) as [
        string,
        TransactionItem,
      ][];
  ...
  describe("cartFromStorage", () => {
    ...
    it("strips blob imageUrl on load", () => {
      const raw = JSON.stringify([
        ["p1", sampleItem({ imageUrl: "blob:http://x/y" })],
      ]);
      const map = cartFromStorage(raw);
  ...
  describe("Dexie and storage fallbacks", () => {
    it("loadCartFromSessionStorageAsync reads from keyVal first", async () => {
      const cart = new Map([
        ["a", sampleItem({ productId: "a", quantity: 2, totalPrice: 20 })],
      ]);
      const db = getDb();
      await db.open();
      await db.keyVal.put({
        key: CART_STORAGE_KEY,
        value: cartToStorage(cart),
      });
