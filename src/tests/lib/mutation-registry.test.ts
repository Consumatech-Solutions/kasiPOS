...
const mocks = vi.hoisted(() => ({
  ...
  categoriesCreate: vi.fn(async (..._args: unknown[]) => ({ id: 'cat1', name: 'C' })),
  categoriesUpdate: vi.fn(async (..._args: unknown[]) => ({})),
  categoriesDelete: vi.fn(async (..._args: unknown[]) => ({})),
  customersCreate: vi.fn(async (..._args: unknown[]) => ({ data: { id: 'cust-srv' } })),
  ...
  categoriesGetAll: vi.fn(async (..._args: unknown[]) => [{ id: 'resolved-cat', name: 'Beverages' }]),
...
vi.mock('@/lib/api/settings', () => ({
  settingsApi: {
    patch: (body: unknown, storeId?: unknown) => mocks.settingsPatch(body, storeId),
  },
}));
...
describe('executeMutation', () => {
  ...
  it('transactions/create maps temp ids via syncIdMapping', async () => {
    await getDb().syncIdMapping.put({ tempId: 'temp-p', serverId: 'real-p', createdAt: Date.now() });
    await getDb().syncIdMapping.put({ tempId: 'temp-cust', serverId: 'real-cust', createdAt: Date.now() });
  ...
  it('products/create uses categoryId and syncIdMapping for _tempId', async () => {
    ...
    expect((await getDb().syncIdMapping.get('temp-prod'))?.serverId).toBe('srv-p1');
  });
  ...
  it('products/update resolves category from name when category set without categoryId', async () => {
    await executeMutation(['products', 'update'], {
      id: 'pid',
      data: { name: 'N', price: 2, costPrice: 0, stock: 1, category: 'Beverages' },
    });
  ...
  it('categories CRUD', async () => {
    ...
    await executeMutation(['categories', 'update'], { id: '1', data: { name: 'Z' } });
  ...
  it('customers CRUD and syncIdMapping on create', async () => {
    ...
    expect((await getDb().syncIdMapping.get('temp-cu'))?.serverId).toBe('cust-srv');
    await executeMutation(['customers', 'update'], { id: '1', data: { name: 'B' } });
  ...
  it('stockAdjustments/create resolves temp product id', async () => {
    await getDb().syncIdMapping.put({ tempId: 'temp-pr', serverId: 'real-pr', createdAt: Date.now() });
  ...
  it('purchaseOrders create and updateStatus', async () => {
    ...
    await executeMutation(['purchaseOrders', 'updateStatus'], { id: 'o1', status: 'completed' });
    expect(mocks.poUpdateStatus).toHaveBeenCalledWith('o1', { status: 'completed' });
  });
  ...
  it('users create, update, delete', async () => {
    ...
    await executeMutation(['users', 'update'], { id: 'u1', data: { name: 'V' } });
  ...
  it('settings/patch calls API', async () => {
    await executeMutation(['settings', 'patch'], { storeId: 'st', body: { receiptHeader: 'H' } });
    expect(mocks.settingsPatch).toHaveBeenCalledWith({ receiptHeader: 'H' }, 'st');
  });
  ...
  it('vouchers CRUD', async () => {
    ...
    await executeMutation(['vouchers', 'update'], { id: 'v1', data: { isActive: false } });
  ...
  it('throws on unknown mutation key', async () => {
    await expect(executeMutation(['unknown', 'op'], {})).rejects.toThrow('Unknown mutation');
  });
});
