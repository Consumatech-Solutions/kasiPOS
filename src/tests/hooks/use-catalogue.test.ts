...
vi.mock('@/lib/catalogue-network-hydrate', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@/lib/catalogue-network-hydrate')>();
  return {
    ...actual,
...
describe('useCategories', () => {
  ...
  it('loads from Dexie when offline', async () => {
    ...
    const { result } = renderHook(() => useCategories(1, 10, { storeIdForOffline: 's1' }), {
  ...
  it('rolls back optimistic create when API fails', async () => {
    ...
    await waitFor(() => expect(result.current.loading).toBe(false));
    expect(result.current.categories).toHaveLength(1);
    await expect(result.current.createCategory({ name: 'NewCat' })).rejects.toThrow('network');
  ...
  it('replaces optimistic row after successful create', async () => {
    ...
    await waitFor(() => expect(result.current.categories.some((c) => c.id === 'srv-cat')).toBe(true));
  });
});
...
describe('useProducts', () => {
  ...
  it('loads from Dexie when offline', async () => {
    ...
    const { result } = renderHook(() => useProducts(1, 10, { storeIdForOffline: 's1' }), {
      wrapper: createWrapper(),
    });
    await waitFor(() => expect(result.current.loading).toBe(false));
