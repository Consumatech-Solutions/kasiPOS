...
import { CartProvider, resetCartMemoryCacheForTests, useCart } from '@/components/providers/cart-provider';
...
function product(p: Partial<Product> & Pick<Product, 'id' | 'name' | 'price'>): Product {
  return { ...p } as Product;
}

function Harness() {
  const { cart, addToCart, updateQuantity, clearCart, cartItemCount, isCartHydrated } = useCart();
  return (
    <div>
      ...
      <button type="button" data-testid="add-p1" onClick={() => addToCart(product({ id: 'p1', name: 'One', price: 12, stock: 3 }))}>
        add
      </button>
      <button type="button" data-testid="add-unlimited" onClick={() => addToCart(product({ id: 'p2', name: 'Two', price: 5 }))}>
        add unlimited
      </button>
      <button type="button" data-testid="inc-qty" onClick={() => updateQuantity('p1', 2)}>
        set qty 2
      </button>
      <button type="button" data-testid="set-qty-5" onClick={() => updateQuantity('p1', 5)}>
        set qty 5
      </button>
      <button type="button" data-testid="remove" onClick={() => updateQuantity('p1', 0)}>
        remove
      </button>
...
describe('CartProvider', () => {
  ...
  it('hydrates then persists cart on change', async () => {
    ...
    const lastCall = saveCart.mock.calls.at(-1)?.[0] as Map<string, { quantity: number }>;
  ...
  it('parses string price on add', async () => {
    function PriceHarness() {
      ...
      return (
        <>
          ...
          <button type="button" onClick={() => addToCart(product({ id: 'x', name: 'X', price: '9.5' as unknown as number }))}>
            go
          </button>
        </>
      );
    ...
    await waitFor(() => expect(screen.queryByRole('button', { name: /go/i })).toBeTruthy());
